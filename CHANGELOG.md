...
* **MFA hardening pass:**
  ...
  * `/login` response now includes `mfa_nonce` on BOTH the existing `mfa_required` shape AND the `mfa_setup_required` shape (alongside `enrollment`). Both `/login/mfa` and `/login/mfa/setup` now require the nonce in the request body and reject mismatches with a generic 422. Defeats two-tab pending-state pollution. **Breaking** for API clients that called either endpoint without echoing the nonce; legacy clients receive 422.
